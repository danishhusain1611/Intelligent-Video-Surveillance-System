    ...
    if len(detections) > 0:  					 
        ...
        for idx, box in centroid_dict.items():  
            if idx in fall_alert_list:  
                cv2.rectangle(img, (box[2], box[3]), (box[4], box[5]), (255, 0, 0), 2) # Create Red bounding boxes 
            else:
...
def gen_frames(): 
    ...
    while True:                                                          # Load the input frame and write output frame.
        ...
        if case == 'object':
            frame_resized = cvDrawBoxes_object(detections, frame_resized)        
        elif case == 'social':
            img = cvDrawBoxes_social(detections, frame_resized)       
        elif case == 'fall':
            frame_resized = cvDrawBoxes_fall(detections, frame_resized)          
        elif case == 'vehicle':
            img = cvDrawBoxes_vehicle(detections, frame_resized)       
